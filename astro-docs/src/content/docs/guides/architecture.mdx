...
- **Unregister & retry** — `manager.unregister(name)` removes a module from the registry (cancelling any pending restart/retry timers) so the name can be re-registered with a freshly built `Module` — e.g. a supervisor retrying after a terminal `SpawnError` with new CLI args. A running module requires `{ force: true }`, which shuts it down first; `register(module, { replace: true })` swaps a non-running module in one call.
