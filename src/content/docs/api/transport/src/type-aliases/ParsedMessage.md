...
> **ParsedMessage**\<`TReq`, `TRes`, `TNotif`\> = \{ `kind`: `"request"`; `message`: `TReq`; \} \| \{ `kind`: `"response"`; `message`: `TRes`; \} \| \{ `kind`: `"notification"`; `message`: `TNotif`; \} \| \{ `error`: [`ProtocolDataError`](/api/transport/src/interfaces/protocoldataerror/); `kind`: `"invalid"`; `raw`: `unknown`; \}
